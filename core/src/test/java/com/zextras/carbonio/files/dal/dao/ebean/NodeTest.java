...
class NodeTest {
  ...
  void givenAllNodeAttributesTheConstructorShouldCreateNodeObjectCorrectly() {
    ...
    Assertions.assertThat(node.getExtension()).isPresent().isEqualTo(Optional.of("png"));
    Assertions.assertThat(node.getDescription())
        .isPresent()
    ...
    Assertions.assertThat(node.getParentId())
        .isPresent()
        .isEqualTo(Optional.of("e2488edb-f408-4468-bb54-21292edc6440"));
    Assertions.assertThat(node.getAncestorIds())
        .isEqualTo("LOCAL_ROOT,e2488edb-f408-4468-bb54-21292edc6440");
    Assertions.assertThat(node.getAncestorsList())
        .isNotEmpty()
  ...
  void givenDifferentNodeAttributesTheSettersShouldUpdateNodeObjectCorrectly() {
    ...
    // When
    node.setOwnerId("c6bf990d-86b9-49ad-a6c0-12260308b7c5")
    ...
    Assertions.assertThat(node.getNodeType()).isEqualTo(NodeType.IMAGE);
    Assertions.assertThat(node.getNodeCategory()).isEqualTo(NodeCategory.FILE);
    Assertions.assertThat(node.getLastEditorId())
        .isPresent()
  ...
  @Test
  void
      givenAFolderNodeTypeTheNodeConstructorShouldInitializeTheNodeCategoryAndTheCurrentVersionCorrectly() {
    // Given & When
    Node node =
        new Node(
            "",
  ...
  @Test
  void
      givenARootNodeTypeTheNodeConstructorShouldInitializeTheNodeCategoryAndTheCurrentVersionCorrectly() {
    // Given & When
    Node node =
        new Node("", "", "", "", 5L, 10L, "LOCAL_ROOT", "", NodeType.ROOT, "LOCAL_ROOT", 0L);
  ...
  @Test
  void
      givenAFolderNodeTheGetExtensionShouldReturnAnOptionalEmptyAndTheFullNameShouldMatchWithTheName() {
    // Given & When
    Node node =
        new Node(
            "",
  ...
  void givenANodeWithoutAnAncestorIdsTheGetAncestorsListShouldReturnAnEmptyList() {
    // Given & When
    Node node = new Node("", "", "", "", 5L, 10L, "test folder", "", NodeType.FOLDER, "", 0L);
