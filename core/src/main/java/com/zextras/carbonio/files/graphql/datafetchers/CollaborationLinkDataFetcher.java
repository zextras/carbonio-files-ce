...
public class CollaborationLinkDataFetcher {
  ...
  private DataFetcherResult<Map<String, Object>> convertCollaborationLinkToDataFetcherResult(
      CollaborationLink collaborationLink, String requesterDomain) {

    String invitationURL =
        MessageFormat.format(
            "{0}{1}{2}",
            requesterDomain, Endpoints.COLLABORATION_LINK_URL, collaborationLink.getInvitationId());
    ...
    linkMap.put(
        GraphQL.CollaborationLink.CREATED_AT, collaborationLink.getCreatedAt().toEpochMilli());
        ...
        CompletableFuture.supplyAsync(
            () -> {
              ...
              SharePermission permissions =
                  environment.getArgument(CreateCollaborationLink.PERMISSION);

              if (permissionsChecker
                  .getPermissions(nodeId, requester.getId().getUserId())
                  .has(permissions)) {

                // If there is an existing collaboration link having the same permission then the
                // system
                ...
                CollaborationLink link =
                    collaborationLinkRepository
                        ...
                        .filter(
                            collaborationLink ->
                                permissions.equals(collaborationLink.getPermissions()))
                        .findFirst()
                        .orElseGet(
                            () ->
    ...
    return environment ->
        CompletableFuture.supplyAsync(
            () -> {
              ...
              ACL permissions =
                  permissionsChecker.getPermissions(nodeId, requester.getId().getUserId());

              if (permissions.has(SharePermission.READ_AND_SHARE)
                  || permissions.has(SharePermission.READ_WRITE_AND_SHARE)) {
                // Before returning the list, the system filters the collaborationLinks the user has
                // no
                // permission to see
                return collaborationLinkRepository
                    .getLinksByNodeId(nodeId)
                    .filter(
                        collaborationLink -> permissions.has(collaborationLink.getPermissions()))
                    .map(
                        collaborationLink ->
                            convertCollaborationLinkToDataFetcherResult(
                                collaborationLink, requester.getDomain()))
  ...
  public DataFetcher<CompletableFuture<DataFetcherResult<List<String>>>>
    ...
    return environment ->
        CompletableFuture.supplyAsync(
            () -> {
                  ...
                  collaborationId -> {
                    boolean isForbidden =
                        collaborationLinkRepository
                            ...
                            .filter(
                                collaborationLink -> {
                                  ACL requesterPermission =
                                      permissionsChecker.getPermissions(
                                          collaborationLink.getNodeId(),
                                          requester.getId().getUserId());

                                  return requesterPermission.has(
                                          SharePermission.READ_WRITE_AND_SHARE)
              ...
              List<UUID> collaborationIdsToDelete =
                  collaborationIds.stream()
                      .filter(
                          collaborationId -> !collaborationIdsForbidden.contains(collaborationId))
                      .map(UUID::fromString)
                      .collect(Collectors.toList());
              ...
              return DataFetcherResult.<List<String>>newResult()
                  .data(
                      collaborationIdsToDelete.stream()
                          .map(UUID::toString)
                          .collect(Collectors.toList()))
                  .errors(
                      collaborationIdsForbidden.stream()
