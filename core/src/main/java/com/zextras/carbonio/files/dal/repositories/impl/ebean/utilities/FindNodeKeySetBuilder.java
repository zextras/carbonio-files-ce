    ...
    sort.ifPresentOrElse(
        s -> {
          ...
          if(nameOfComparison.equals(Files.Db.Node.SIZE)){
            ...
            result.set(compareExpression(nameOfComparison, symbolOfComparison, valueOfComparison)
                .or(
                    compareExpression(nameOfComparison, "=", valueOfComparison)
                        .and(
                            compareExpression("t0.node_id", ">", formattedIdToCompare)
                        )
                )
                .toExpression());
          } else {
            ...
            if(nameOfComparison.equals(Files.Db.Node.NAME)){
              //Special case: if sorting by name we actually want to compare lowercase names, so get name and adapt query composition
            ...
            String formattedValueOfComparison = objOfComparison instanceof String ? "'" + objOfComparison + "'" : objOfComparison.toString(); //add quotes if value of comparison is a string

            result.set(compareExpression(Files.Db.Node.CATEGORY, ">", nodeCategory)
                .or(compareExpression(Files.Db.Node.CATEGORY, "=", nodeCategory)
                    .and(compareExpression(formattedNameOfComparison, symbolOfComparison, formattedValueOfComparison))
                ).or(
