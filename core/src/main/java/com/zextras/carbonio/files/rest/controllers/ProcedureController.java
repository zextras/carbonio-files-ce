...
public class ProcedureController extends SimpleChannelInboundHandler<FullHttpRequest> {
  ...
  public ProcedureController(
      ProcedureService procedureService, PermissionsChecker permissionsChecker) {
  ...
  protected void channelRead0(ChannelHandlerContext context, FullHttpRequest httpRequest) {
    ...
    logger.error(
        MessageFormat.format(
            "Request {0} {1}: bad request", httpRequest.method(), httpRequest.uri()));
   ...
   */
  private void uploadToModule(ChannelHandlerContext context, FullHttpRequest httpRequest) {

    final UserMyself requester =
        (UserMyself) context.channel().attr(AttributeKey.valueOf(ContextAttribute.REQUESTER)).get();

    final String requesterCookies =
        (String) context.channel().attr(AttributeKey.valueOf(ContextAttribute.COOKIES)).get();
    ...
    try {
      bodyRequest =
          new ObjectMapper()
              .readValue(
                  httpRequest.content().toString(StandardCharsets.UTF_8), UploadToRequest.class);
    ...
    if (permissionsChecker
      ...
      procedureService
          ...
          .onSuccess(
              attachmentId -> {
                try {
                  UploadAttachmentResponse bodyResponse =
                      new UploadAttachmentResponse(attachmentId);
                  ...
                  HttpResponse httpResponse =
                      new DefaultFullHttpResponse(
                          ...
                          Unpooled.wrappedBuffer(
                              new ObjectMapper().writeValueAsBytes(bodyResponse)),
                          headers,
      ...
      logger.error(
          MessageFormat.format(
              ...
              httpRequest.uri(), bodyRequest.getNodeId()));
