...
public class GraphQLController extends SimpleChannelInboundHandler<FullHttpRequest> {
   ...
   */
  private static final ChannelFutureListener sNettyChannelFutureClose =
      (promise) -> {
   ...
   *
   * <ul>
   *   <li>Creates a Runnable implementing the run method that executes the channelRead_async()
   *       (which handles the request).
   *   <li>Puts the created runnable into the ActivityManager.
   * </ul>
   *
   * @param context
   * @param httpRequest
  ...
  private void channelReadAsync(ChannelHandlerContext context, FullHttpRequest httpRequest) {
    ...
    if (httpRequest.headers().contains("Internal")) {
      graphQLContext.put(Constants.GraphQL.Context.INTERNAL, httpRequest.headers().get("Internal"));
    }
    ...
    try {
      ...
      JsonNode jsonResponse =
          new ObjectMapper().createObjectNode().put("error_message", "Something went wrong");
      ...
      FullHttpResponse response =
          new DefaultFullHttpResponse(
              ...
              Unpooled.wrappedBuffer(
                  jsonResponse.toPrettyString().getBytes(StandardCharsets.UTF_8)));
      response.headers().add(HttpHeaderNames.CONTENT_TYPE, HttpHeaderValues.APPLICATION_JSON);
      response.headers().add(HttpHeaderNames.CONTENT_LENGTH, response.content().readableBytes());
   ...
   * multiple elements in batch. <strong>Note that the data loaders must be created per execution
   * request.</strong>
   *
  ...
  private DataLoaderRegistry buildDataLoaderRegistry() {
    ...
    // DataLoaderRegistry is a place to register all data loaders in that needs to be dispatched
    // together
