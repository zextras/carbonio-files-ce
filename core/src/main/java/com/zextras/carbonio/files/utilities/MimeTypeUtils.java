...
public class MimeTypeUtils {
  ...
  static {
     ...
     * To update the list, just use this command
     * while IFS= read -r line; do long_var_name="$(echo "${line}" | awk '{printf $1}')"; [[ "${line}" != "${long_var_name}" ]] && for i in ${line}; do [[ "${i}" != "${long_var_name}" ]] && echo "mimeTypes.put(\"${i}\", \"${long_var_name}\");"; done; done <<< "$(grep -v "\#" /etc/mime.types)" > /tmp/lol2
     */
   ...
   * @param allowedMimeType is a {@link Set} used to filter the mimetype list that will be
