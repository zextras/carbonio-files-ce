...
public class GraphQLRequest {
  ...
  public GraphQLRequest(String requestType, String request) {
    this(requestType, request, Optional.empty(), new HashMap<>());
  }
   ...
   *
   * <p>This is an example of a complete payload:
   *
   * <pre>
   *  {
   ...
   * </pre>
   *
   * where the <code>query</code> must be always present, but the <code>operationName</code> and the
   * <code>variables</code> can be both optional or only one of the two.
   *
   * <h2>Execution step by step</h2>
   *
   * <ul>
   *   <li>It builds a {@link Map} from the payload string.
   *   <li>It Extracts the type and the json of the request from the payload. If the payload doesn't
   *       contain a correct request type or has an empty body it throws an {@link
   *       InvalidPayloadRequestError}.
   *   <li>It Checks if the operation name is specified into the body of the request, if there is an
   *       operation name it will be extracted.
   *   <li>It Checks if the set of variables is specified into the body of the request, if there is
   *       a set of variables it will be extracted.
   * </ul>
   *
  ...
  public static final GraphQLRequest buildFromPayload(String payloadString)
    ...
    Optional<String> requestType =
        Arrays.stream(RequestTypes.values())
   ...
   *
   * <ul>
   *   <li><strong>QUERY</strong>: represents a read-only request
   *   <li><strong>MUTATION</strong>: represents a write request but returns also the specified
   *       attributes of the object modified.
   *   <li><strong>SUBSCRIPTION</strong>: like the QUERY but the result is sent back to the client
