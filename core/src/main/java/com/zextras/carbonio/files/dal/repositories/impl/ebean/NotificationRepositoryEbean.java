...
public class NotificationRepositoryEbean implements NotificationRepository {
  ...
  @Inject
  public NotificationRepositoryEbean(DatabaseManager databaseManagerFlyway) {
  ...
  public ImmutablePair<List<BaseNotification>, String> getNotifications(
    ...
    return new ImmutablePair<>(notifications, createPageToken(notifications, limit.orElse(25)));
  }

  // This page token is really simple and definitely does not need the complexity of the page token
  // used for the
  // findNodes. It just is the id of the last notification returned, so the next query can start
  // from there.
  ...
  private List<BaseNotification> doFind(String userId, Integer limit, Optional<String> pageToken) {
    ...
    // we decode it and get the notifications from the last notification returned (it's id is inside
    // the token);
    ...
    pageToken.ifPresentOrElse(
                  ...
                  userNotification -> {
                    ...
                    notificationIdsAtomic.set(
                        mDB
                            ...
                            .eq(Constants.Db.UserNotificationInterest.USER_ID, userId)
                            .lt(
                                Constants.Db.UserNotificationInterest.CREATED_AT,
                                lastNotificationCreatedAt)
                            .orderBy()
                            .desc(Constants.Db.UserNotificationInterest.CREATED_AT)
                            .orderBy()
                            .desc(
                                Constants.Db.UserNotificationInterest
                                    .NOTIFICATION_ID) // discriminate on equal timestamps
                            .setMaxRows(limit)
                            .findList()
                            .stream()
        ...
        () -> // Limit and order before joining, so from here we have max N rows from pagination and
            // we can write
    ...
    // Here we get generic notification objects; since we can't have inheritance we manually fetch
    ...
    Map<NotificationType, List<String>> groupedIds =
              ...
              if (idsForType != null && !idsForType.isEmpty()) {
                ...
                List<? extends BaseNotification> resultList =
                    mDB.getEbeanDatabase()
                        ...
                        .findList();
                resultList.forEach(
                    notif -> notificationsById.put(notif.getNotificationId(), notif));
              }
            });

    // We keep initial order, so they are already ordered by createdAt
    return notificationIds.stream().map(notificationsById::get).collect(Collectors.toList());
  }

  @Override
  public Optional<UserNotificationsInfo> getUserNotificationsInfo(String userId) {
    return mDB.getEbeanDatabase()
        .find(UserNotificationsInfo.class)
        .where()
        .idEq(userId)
        .findOneOrEmpty();
  }
        ...
        .ifPresentOrElse(
            snap -> {
              ...
              if (snap.representNode(node)) {
                ...
                logger.debug(
                    "Found snapshots but not equal to node, creating a new snapshot for node {}",
                    node.getId());
            ...
            () -> {
              logger.debug(
                  "Not found snapshots for node, creating a new snapshot for node {}",
                  node.getId());
              snapshotNodeRef.set(createSnapshottedNode(node));
            });
  ...
  private SnapshotUser conditionallySnapshotUser(UserMyself user) {
    ...
    getLatestSnapshotOfUser(user.getId().getUserId())
        .ifPresentOrElse(
            snap -> {
              // check if identical, if not still create a snapshot
              if (snap.representUser(user)) {
                ...
                logger.debug(
                    "Found snapshots but not equal to user, creating a new snapshot for user {}",
                    user.getId());
  ...
  private BaseNotification handleNewNotification(
  ...
  // UserNotificationsInfo for all interest users if not present, while increasing their unread
  ...
  public NewShareNotification createNewShareNotification(
    ...
    NewShareNotification newShareNotification =
    ...
    // THIRD STEP: SET USER TO NOTIFY, UPDATING THEIR UNREAD COUNTER (AND CREATING USER INFO IF NOT
    // PRESENT)
  ...
  public AddedNodeNotification createAddedNodeNotification(
    ...
    // SECOND STEP: CREATE THE NOTIFICATION
    AddedNodeNotification addedNodeNotification =
  ...
  public RemovedNodeNotification createRemovedNodeNotification(
    ...
    // SECOND STEP: CREATE THE NOTIFICATION
    RemovedNodeNotification removedNodeNotification =
    ...
    // THIRD STEP: SET USER TO NOTIFY, UPDATING THEIR UNREAD COUNTER (AND CREATING USER INFO IF NOT
    // PRESENT)
    return (RemovedNodeNotification)
        handleNewNotification(removedNodeNotification, usersIdsToNotify);
  }
}
