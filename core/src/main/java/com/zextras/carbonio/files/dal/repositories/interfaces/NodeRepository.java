...
/**
 * This is the only class allowed to execute CRUD operations on a {@link Node} element.
 *
 * <p>It's method must be implemented by specific implementations depending on DB and ORM used.
 */
public interface NodeRepository {

  /**
   * Allows to retrieve a {@link Node} from the database or from the cache if it was recently
   * requested.
   *
   * <p>First it checks if the {@link Node} is on the database and then it returns a {@link Node}.
   *
  ...
  /**
   * Retrieves a {@link Node} from the database with a row-level lock (SELECT FOR UPDATE).
   *
   * <p>This method MUST be called within an active transaction. The lock is held until the
   * transaction commits or rolls back, preventing concurrent modifications to the same node.
   *
   * <p>Use this method when you need to read a node and then update it atomically, such as when
   ...
   *     will search on the whole subtree, if false it will search only direct children of the
   *     folder
  ...
  /**
   * Allows to retrieve the list of {@link Node}s from the database or from the cache if already
   * present.
   *
   * <p>The retrieved list can be sorted using one or more {@link NodeSort}.
   *
   ...
   */
  Stream<Node> getNodes(List<String> nodeIds, Optional<NodeSort> sort);

  /**
   * Allows to retrieve the list of ids relative to the children of a specific {@link Node}.
   *
   * <p>The retrieved list can be sorted using one or more {@link NodeSort}.
   *
  ...
  /**
   * Creates a new {@link Node} and, after saving it in the database, it returns the {@link Node}
   * just created.
   *
  ...
  /**
   * Deletes a {@link Node} from the database and from the cache if enabled.
   *
   * <p>if the {@link Node} does not exist, then the method does nothing and returns <code>false
   * </code>.
   *
  ...
  /**
   * Changes the flag custom attribute for a {@link Node}.
   *
   * <p>if the {@link NodeCustomAttributes} does not exist, then the method creates the custom
   * attributes on the database for the {@link Node} and the user .
   *
   ...
   */
  void flagForUser(String nodeId, String userId, boolean flag);

  /**
   * Returns the flag value for the couple {@link Node} and user.
   *
   * @param nodeId is a {@link String} representing the id of the node to flag
   * @param userId is a {@link String} representing the id of the user for which we have to flag the
   *     node
   */
  boolean isFlaggedForUser(String nodeId, String userId);
  ...
  /**
   * Trashes a node putting it into the TRASHED table.
   *
   ...
   */
  void trashNode(String nodeId, String parentId);

  /**
   * Untrashes a node putting it into its original position if it still exists. Otherwise it will
   * put it in the LOCAL_ROOT.
   *
   * @param nodeId is a {@link String} representing the id of the node to trash
   */
  ...
  /**
   * Moves a list of {@link Node}s to a destination folder.
   *
   ...
   *     purposes.
   */
  int moveNodes(List<String> nodesIds, Node destinationFolder);

  /**
   * Updates a {@link Node} on database.
   *
  ...
  /**
   * Deletes from the database all nodes marked for deletion whose retention time exceeded the value
   * passed as parameter.
   *
   * @param retentionTimestamp represents the number of milliseconds that a {@link Node} can pass in
   *     trash before being deleted
   * @return a boolean which is <code>true</code> if the deletion was performed correctly, <code>
   *     false</code> otherwise
   */
  int deleteTrashedNodesOlderThan(Long retentionTimestamp);

  /**
   * Returns all Nodes trashed.
   *
  ...
  /**
   * Returns a {@link List} containing all the root folders.
   *
   ...
   */
  Optional<Node> getNodeByName(String nodeName, String folderId, String nodeOwner);

  /**
   * @param ownerId is the owner id from which to return nodes.
   * @return a {@link List<Node>} representing all nodes owned by ownerId.
   */
  List<Node> findNodesByOwner(String ownerId);

  /**
   * @param ownerId is the owner id from which to return node.
   *     <p>Returns the first {@link Node} of all nodes owned by ownerIds.
   */
  ...
  /** Returns a {@link List<Node>} with absolutely all the nodes ever saved. */
