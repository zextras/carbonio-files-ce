...
public class NotificationDataFetcher {
  ...
  public NotificationDataFetcher(
      FilesConfig filesConfig, NotificationRepository notificationRepository) {
    ...
    return environment -> {
      ...
      return switch (type) {
        case NotificationTypeCodes.NEW_SHARE ->
            environment.getSchema().getObjectType(Constants.GraphQL.Types.NEW_SHARE);
  ...
  private DataFetcherResult<Map<String, Object>> convertNotificationToPageResult(
    ...
    switch (type) {
      ...
      default -> {
        /* Unknown/Advanced-only type: base fields already populated. */
      }
    }
