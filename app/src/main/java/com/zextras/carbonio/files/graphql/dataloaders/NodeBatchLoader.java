...
public class NodeBatchLoader implements BatchLoader<String, Try<Node>> {
  ...
  public CompletionStage<List<Try<Node>>> load(List<String> nodeIds) {
    ...
    // hopping to ForkJoinPool.commonPool via supplyAsync. The DataLoader dispatch is driven from
    // inside graphQL.execute() on the Vert.x worker thread where the CDI request context (and thus
    // the request-scoped EntityManager) is active; running the batch query there avoids the
    // ContextNotActiveException that a commonPool thread would hit.
    ...
    nodeIdsNotFound.forEach(nodeIdNotFound -> results.add(Try.failed(new NodeNotFoundException())));
