 ...
 * handle and execute every single request. In fact it knows:
 *
 * <ul>
...
public class GraphQLProvider {
   ...
   *       batching mechanism using {@link org.dataloader.DataLoader}s
   * </ul>
   *
   * @return {@link GraphQL}
   */
  private GraphQL setup() {
    List<Instrumentation> chainedInstrumentations =
        List.of(buildValidationInstrumentation(), new MaxQueryDepthInstrumentation(10));
  ...
  private FieldValidationInstrumentation buildValidationInstrumentation() {
    SimpleFieldValidation fieldValidation =
   ...
   * <ul>
   *   <li>Interface
   *   <li>Relation between different types (for example: Node has a relation with the Share type)
   *   <li>Query
   *   <li>Mutation
   * </ul>
   *
  ...
  private RuntimeWiring buildWiring() {
    RuntimeWiring.Builder builder =
        RuntimeWiring.newRuntimeWiring()
            ...
            .type(
                newTypeWiring(Constants.GraphQL.Types.NODE_SORT)
                    .enumValues(nodeDataFetcher.getNodeSortResolver()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.SHARE_PERMISSION)
                    .enumValues(shareDataFetcher.getSharePermissionsResolver()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.NODE_TYPE)
                    .enumValues(nodeDataFetcher.getNodeTypeResolver()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.ADDED_NODE_TYPE)
                    .enumValues(notificationDataFetcher.getAddedNodeTypeResolver()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.REMOVED_NODE_TYPE)
                    .enumValues(notificationDataFetcher.getRemovedNodeTypeResolver()))
            .type(
                newTypeWiring("Query")
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_NODE, nodeDataFetcher.getNodeFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_USER, userDataFetcher.getUserFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_SHARE, shareDataFetcher.getShareFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_ROOTS_LIST,
                        nodeDataFetcher.getRootsListFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_PATH, nodeDataFetcher.getPathFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Queries.FIND_NODES, nodeDataFetcher.findNodesFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_VERSIONS,
                        nodeDataFetcher.getVersionsFetcher())
                    .dataFetcher(
                    ...
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_CONFIGS, configDataFetcher.getConfigs())
                    .dataFetcher(
                        Constants.GraphQL.Queries.GET_NOTIFICATIONS,
                        notificationDataFetcher.getNotificationsFetcher()))
            .type(
                newTypeWiring("Mutation")
                    .dataFetcher(
                        Constants.GraphQL.Mutations.CREATE_FOLDER,
                        nodeDataFetcher.createFolderFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.UPDATE_NODE,
                        nodeDataFetcher.updateNodeFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.FLAG_NODES, nodeDataFetcher.flagNodes())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.TRASH_NODES, nodeDataFetcher.trashNodes())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.RESTORE_NODES, nodeDataFetcher.restoreNodes())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.MOVE_NODES, nodeDataFetcher.moveNodesFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.DELETE_NODES,
                        nodeDataFetcher.deleteNodesFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.DELETE_VERSIONS,
                        nodeDataFetcher.deleteVersionsFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.KEEP_VERSIONS,
                        nodeDataFetcher.keepVersionsFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.CLONE_VERSION,
                        nodeDataFetcher.cloneVersionFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.COPY_NODES, nodeDataFetcher.copyNodesFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.CREATE_SHARE,
                        shareDataFetcher.createShareFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.UPDATE_SHARES,
                        shareDataFetcher.updateSharesFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.DELETE_SHARES,
                        shareDataFetcher.deleteSharesFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.CREATE_LINK, linkDataFetcher.createLink())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.UPDATE_LINK, linkDataFetcher.updateLink())
                    .dataFetcher(
                        Constants.GraphQL.Mutations.DELETE_LINKS, linkDataFetcher.deleteLinks())
                    .dataFetcher(
            ...
            .type(
                newTypeWiring(Constants.GraphQL.Types.NODE_INTERFACE)
                    .typeResolver(nodeDataFetcher.getNodeInterfaceResolver()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.FILE)
                    .dataFetcher(
                        Constants.GraphQL.FileVersion.CREATOR, userDataFetcher.getUserFetcher())
                    .dataFetcher(
                        Constants.GraphQL.FileVersion.OWNER, userDataFetcher.getUserFetcher())
                    .dataFetcher(
                        Constants.GraphQL.FileVersion.LAST_EDITOR, userDataFetcher.getUserFetcher())
                    .dataFetcher(
                        Constants.GraphQL.FileVersion.PARENT, nodeDataFetcher.getNodeFetcher())
                    .dataFetcher(
                        Constants.GraphQL.FileVersion.PERMISSIONS,
                        nodeDataFetcher.getPermissionsNodeFetcher())
                    .dataFetcher(
                        Constants.GraphQL.FileVersion.SHARES, shareDataFetcher.getSharesFetcher())
                    ...
                    .dataFetcher(
                        ...
                        collaborationLinkDataFetcher.getCollaborationLinksByNodeId()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.FOLDER)
                    ...
                    .dataFetcher(
                        Constants.GraphQL.Folder.LAST_EDITOR, userDataFetcher.getUserFetcher())
                    .dataFetcher(Constants.GraphQL.Folder.PARENT, nodeDataFetcher.getNodeFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Folder.CHILDREN,
                        nodeDataFetcher.getChildNodesFetcherFast())
                    .dataFetcher(
                        Constants.GraphQL.Folder.PERMISSIONS,
                        nodeDataFetcher.getPermissionsNodeFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Folder.SHARES, shareDataFetcher.getSharesFetcher())
                    .dataFetcher(Constants.GraphQL.Folder.LINKS, linkDataFetcher.getLinks())
                    .dataFetcher(
                        Constants.GraphQL.Folder.COLLABORATION_LINKS,
                        collaborationLinkDataFetcher.getCollaborationLinksByNodeId()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.NODE_PAGE)
                    .dataFetcher(
                        Constants.GraphQL.NodePage.NODES, nodeDataFetcher.nodePageFetcher()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.SHARED_TARGET)
                    .typeResolver(userDataFetcher.getAccountTypeResolver()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.ACCOUNT)
                    .typeResolver(userDataFetcher.getAccountTypeResolver()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.SHARE)
                    .dataFetcher(Constants.GraphQL.Share.NODE, nodeDataFetcher.sharedNodeFetcher())
                    .dataFetcher(
                        Constants.GraphQL.Share.SHARE_TARGET,
                        userDataFetcher.shareTargetUserFetcher()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.DISTRIBUTION_LIST)
                    .dataFetcher(
                        Constants.GraphQL.DistributionList.USERS,
                        userDataFetcher.getDLUsersFetcher()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.LINK)
                    .dataFetcher(Constants.GraphQL.Link.NODE, nodeDataFetcher.sharedNodeFetcher()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.COLLABORATION_LINK)
                    .dataFetcher(Constants.GraphQL.Link.NODE, nodeDataFetcher.sharedNodeFetcher()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.NOTIFICATION_PAGE)
                    .dataFetcher(
                        Constants.GraphQL.NotificationPage.NOTIFICATIONS,
                        notificationDataFetcher.notificationPageFetcher()))
            .type(
                newTypeWiring(Constants.GraphQL.Types.NOTIFICATION)
                    .typeResolver(notificationDataFetcher.getNotificationInterfaceResolver()))
            // Since we always return the nested objects with the notification call, no need to
            // fetch them again (return as is)
            .type(
                Constants.GraphQL.Types.NEW_SHARE,
                typeWiring ->
                    typeWiring
                        .dataFetcher(
            ...
            .type(
                newTypeWiring(Constants.GraphQL.Types.ADDED_NODE)
                    .dataFetcher(
            ...
            .type(
                newTypeWiring(Constants.GraphQL.Types.REMOVED_NODE)
                    .dataFetcher(
  ...
  private GraphQLSchema buildSchema(RuntimeWiring wiring) {
    ...
    GraphQLCodeRegistry updatedCodeRegistry =
        existingCodeRegistry.transform(builder -> builder.fieldVisibility(blockedFields));

    // Apply code registry to schema
    return graphQLSchema.transform(builder -> builder.codeRegistry(updatedCodeRegistry));
  }
