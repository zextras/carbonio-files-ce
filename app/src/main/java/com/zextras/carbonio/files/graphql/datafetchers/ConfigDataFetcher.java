 ...
 * {@link HashMap} containing the data fetched from configuration.</p>
...
public class ConfigDataFetcher {
  ...
  public DataFetcher<CompletableFuture<List<DataFetcherResult<Map<String, String>>>>> getConfigs() {
    return environment -> SyncCompletableFuture.supplyAsync(() -> {
      ...
      // Legacy parity: parsed inline with no try/catch, so a malformed override surfaces as an
      // uncaught GraphQL execution error here (unlike the safe FilesConfig#getMaxNumberOfVersions
      ...
      result.add(convertConfigToGraphQLMap(
