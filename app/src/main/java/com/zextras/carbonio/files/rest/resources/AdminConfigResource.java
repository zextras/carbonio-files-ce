 ...
 *             (account &gt; cos &gt; default), each with the {@code source} tier that produced it.
 ...
 *       precedence (no re-implementation here); the user's cos comes from {@link
 ...
 *             application.properties); read-only, complete.
 *         <li>{@code GET /admin/config/raw/{scope}/{scopeId}} — the RAW overrides set at exactly
 *             that scope (scope = account|cos); keys not overridden are absent.
...
public class AdminConfigResource {
  ...
  /**
   * A resolved value together with the scope tier that produced it (account|cos|default). The value
   * may be {@code null} when the winning tier's value is empty (an empty value is itself the base
   * default, not an absence); the source is always one of those three tiers.
   */
  ...
  public RestResponse<Map<String, ResolvedEntry>> getResolvedConfig(
    ...
    for (String k : keys) {
      ConfigResolver.Resolution res = configResolver.resolve(accountId, cosId, k);
      resolved.put(
          k,
          new ResolvedEntry(
  ...
  public RestResponse<Map<String, String>> getDefaultConfig(
    ...
    // The DEFAULT tier ONLY (namespaced application.properties); null when the base default is
    // empty/unset.
  ...
  private Scope parseScope(String scope) {
    try {
      ...
      throw badRequest("Unknown scope '" + scope + "' (expected account|cos)");
    }
  }
