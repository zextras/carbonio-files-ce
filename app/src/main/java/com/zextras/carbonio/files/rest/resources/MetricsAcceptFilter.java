 ...
 * defaults to serving the newer OpenMetrics text exposition ({@code
 * application/openmetrics-text; version=1.0.0}) whenever the request carries no {@code Accept}
 * header, and only falls back to the classic Prometheus text format ({@code text/plain;
 * version=0.0.4}) when the client explicitly asks for {@code text/plain} (or {@code text/html}).
 * The legacy Netty scrape endpoint only ever served the classic text format, and real scrapers
 * (and this acceptance suite) hit {@code /metrics} with no {@code Accept} header at all.
 *
 ...
 * metrics route and injects {@code Accept: text/plain} only when the client did not already state
 * a preference — an explicit {@code Accept} (e.g. a scraper that really wants OpenMetrics) is left
 * untouched.
 */
@ApplicationScoped
