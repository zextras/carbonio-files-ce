...
/**
 * P5: Quarkus/{@code quarkus-scheduler} port of the legacy Guice {@code PurgeService}, which used
 * a raw {@link java.util.concurrent.ScheduledExecutorService}.
 *
 * <p>{@link #purgeTrashedNodes} and {@link #purgeTombstones} are individually {@code
 * @Transactional} (and package-private, like the legacy version) so integration tests can invoke
 * either directly, without going through the scheduler, inside its own transaction/{@code
 ...
 * transaction.
 *
 * <p>{@code quarkus.scheduler.enabled=false} in {@code %test} (see {@code
 * application.properties}) keeps the scheduler from firing during the DAL/GraphQL/REST/purge
 * integration tests, which invoke {@link #purgeTrashedNodes}/{@link #purgeTombstones} directly.
 */
@ApplicationScoped
