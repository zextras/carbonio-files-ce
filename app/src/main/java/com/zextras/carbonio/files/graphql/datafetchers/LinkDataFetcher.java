 ...
 * defined in the GraphQL schema that are related to the {@link Constants.GraphQL.Link} type.
 *
 ...
 * of the related Link's attribute defined in the GraphQL schema.
 *
 ...
 *       error because it cannot find the mandatory attributes related to the Link inside the {@link
 *       Map}.
...
public class LinkDataFetcher {
  ...
  private DataFetcherResult<Map<String, Object>> convertLinkToGraphQLMap(
      Link link, String requesterDomain, boolean isNodeAFolder) {
    ...
    link.getAccessCode().ifPresent(accessCode -> result.put(GraphQL.Link.ACCESS_CODE, accessCode));
  ...
  /**
   * This {@link DataFetcher} must be used for the {@link Constants.GraphQL.Mutations#CREATE_LINK}
   * mutation.
   *
   * <p>The request must have the following parameters in input:
   *
   * <ul>
   *   <li>{@link Constants.GraphQL.InputParameters.Link#NODE_ID}: a {@link String} representing the
   *       id of the node (this is mandatory).
   *   <li>{@link Constants.GraphQL.InputParameters.Link#DESCRIPTION}: a {@link String} representing
   *       the description of the link to create (this is optional).
   *   <li>{@link Constants.GraphQL.InputParameters.Link#EXPIRES_AT}: a long representing the
   *       expiration timestamp.
   * </ul>
   *
   * <h2>Behaviour:</h2>
   *
   * <p>It creates the link with the values specified in input, it saves the mandatory parameter
   * necessary to fetch the related {@link Constants.GraphQL.Node} object, then it creates the
   * GraphQL map of the new link.
   *
   ...
   *     Constants.Config.Link#MAX_LINKS_PER_NODE} links
    ...
    return environment ->
        SyncCompletableFuture.supplyAsync(
            () -> {
              ...
              String nodeId =
                  environment.getArgument(Constants.GraphQL.InputParameters.Link.NODE_ID);
        ...
        SyncCompletableFuture.supplyAsync(
            () -> {
              ...
              UserMyself requester =
                  environment.getGraphQlContext().get(Constants.GraphQL.Context.REQUESTER);
              String linkId =
                  environment.getArgument(Constants.GraphQL.InputParameters.Link.LINK_ID);
              ...
              return linkRepository
                  ...
                  .map(
                      link -> {
                        ...
                        Optional<Long> optNewExpirationTimestamp =
                            Optional.ofNullable(
                                environment.getArgument(InputParameters.Link.EXPIRES_AT));
                        Optional<String> optNewDescription =
                            Optional.ofNullable(
                                environment.getArgument(InputParameters.Link.DESCRIPTION));
                        Optional<String> optNewAccessCode =
                            Optional.ofNullable(
                                environment.getArgument(InputParameters.Link.ACCESS_CODE));
