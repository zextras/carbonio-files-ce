...
class KeyValueChangedHandlerTest {
  ...
  /**
   * This tests that only the correct versions are deleted and results in the corner case where the
   * remaining file versions are more than the new maxversionnumber: this is because some of them
   * are keepforever, and we can't ever delete the current version. While this is a corner case,
   * this test implicitly covers also the generic behaviour since this corner case is just a
   * particular ending situation caused by the normal flow of deleting the least recent file
   * versions.
   */
  @Test
  void givenMaxVersionNumberEventAndExceedingFileVersionsDoHandleShouldDeleteCorrectVersions() {
    ...
    FileVersion fileVersion2 =
        mock(FileVersion.class); // should delete as it is the least recent that is not keepforever
    FileVersion fileVersion3 = mock(FileVersion.class); // should not delete as it is keepforever
    FileVersion fileVersion4 =
        mock(FileVersion.class); // should not delete as it is current version
    ...
    when(fileVersionRepository.getFileVersionsRelatedToNodesHavingVersionsGreaterThan(anyInt()))
        .thenReturn(
            Map.of("key", Arrays.asList(fileVersion1, fileVersion2, fileVersion3, fileVersion4)));

    KeyValueChangedConsumer keyValueChangedConsumer =
        new KeyValueChangedConsumer(fileVersionRepository);

    // When
    keyValueChangedConsumer.doHandle(
        new KeyValueChanged("carbonio-files/max-number-of-versions", "1"));
