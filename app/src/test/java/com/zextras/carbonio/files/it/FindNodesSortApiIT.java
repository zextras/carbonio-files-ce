 ...
 * bug, and {@code ROOT}-category unreachability, both pinned as the ACTUAL behaviour) are preserved
...
class FindNodesSortApiIT extends AbstractFilesIT {
  ...
  private String[] seedThreeFilesWithDistinctOwnersVisibleToRequester() {
    String aId =
        seedFile(
            "ownedByA.txt", LOCAL_ROOT, "a".getBytes(StandardCharsets.UTF_8), REQUESTER_COOKIE);
    tickClock();
    String bId =
        seedFile("ownedByB.txt", LOCAL_ROOT, "b".getBytes(StandardCharsets.UTF_8), OWNER_B_COOKIE);
    seedShare(bId, REQUESTER_ID, ACL.SharePermission.READ_ONLY, OWNER_B_COOKIE);
    tickClock();
    String cId =
        seedFile("ownedByC.txt", LOCAL_ROOT, "c".getBytes(StandardCharsets.UTF_8), OWNER_C_COOKIE);
  ...
  void givenNodesWithDistinctOwnersSortOwnerAscShouldOrderByOwnerIdAscending() {
    ...
    Assertions.assertThat(nodes)
        ...
        .containsExactly(ids[0], ids[1], ids[2]);
  }
  ...
  /**
   * All three files are owned by the SAME user (the requester) so that {@code OWNER} plays no role
   * in the ordering; only {@code editor_id} differs, set via the real {@code updateNode} mutation
   * (which sets {@code Node#setLastEditorId(requesterId)}) run as three different requesters who
   * each hold at least {@code READ_AND_WRITE} on the node.
   */
  private String[] seedThreeFilesWithDistinctLastEditors() {
    String dId =
        seedFile(
            "editedByA.txt", LOCAL_ROOT, "d".getBytes(StandardCharsets.UTF_8), REQUESTER_COOKIE);
    tickClock();
    String eId =
        seedFile(
            "editedByB.txt", LOCAL_ROOT, "e".getBytes(StandardCharsets.UTF_8), REQUESTER_COOKIE);
    seedShare(eId, OWNER_B, ACL.SharePermission.READ_AND_WRITE, REQUESTER_COOKIE);
    tickClock();
    String fId =
        seedFile(
            "editedByC.txt", LOCAL_ROOT, "f".getBytes(StandardCharsets.UTF_8), REQUESTER_COOKIE);
  ...
  private String[] seedOneFolderAndOneFile() {
    ...
    tickClock();
    String fileId =
        seedFile("aFile.txt", LOCAL_ROOT, "f".getBytes(StandardCharsets.UTF_8), REQUESTER_COOKIE);
  ...
  /**
   * FINDING: see the class-level Javadoc. {@code sort: TYPE_DESC} does NOT reverse the folder/file
   * grouping — {@code getRealSortingsToApply} always prepends {@code TYPE_ASC} first, making the
   * requested {@code TYPE_DESC} a no-op secondary clause on the same already-resolved column. The
   * observed order is identical to {@code TYPE_ASC}: folder still before file.
   */
  @Test
   ...
   * GraphQL schema's {@code NodeSort} enum (only {@code LAST_EDITOR}, {@code NAME}, {@code OWNER},
   * {@code TYPE}, {@code UPDATED_AT}, {@code SIZE} are declared). Selecting it over the API can
   * therefore only ever fail GraphQL document validation — it is not a reachable sort.
   */
  @Test
