...
public interface Mocks {
   ...
   * AND (since {@code NodeDataFetcher} now reads {@code FilesConfig#getMaxNumberOfVersions()} live
   * per-operation instead of snapshotting it at construction) the {@code keepVersions}/{@code
   * cloneVersion} cap checks as well — same underlying override as the builder's {@code
   * withMaxNumberOfVersions(int)}, which just calls this on the already-running app.
   */
