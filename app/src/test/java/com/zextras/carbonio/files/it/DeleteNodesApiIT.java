...
class DeleteNodesApiIT extends AbstractFilesIT {
  ...
  private List<String> deletedIds(Response response) {
    return (List<String>)
        TestUtils.jsonResponseToValue(response.getBody().asString(), "deleteNodes").orElse(List.of());
  }
  ...
  @Test
  void givenTwoFilesAndAllBlobsSucceedThenBothAreDeletedAndReturnedInResponse() throws SQLException {
    // Given
    String file1Id =
    ...
    Assertions.assertThat(tombstoneRowsForNode(file1Id) + tombstoneRowsForNode(file2Id)).isEqualTo(0);
  }
  ...
  void givenTwoFilesAndPowerStoreFailsThenBothAreStillDeletedFromDbAndNoErrorReturnedToUser()
    ...
    Assertions.assertThat(tombstoneRowsForNode(file1Id) + tombstoneRowsForNode(file2Id)).isEqualTo(2);
  }
  ...
  @Test
  void givenNullResponseFromPowerStoreThenNodeDeletedButTombstoneKeptForRetry() throws SQLException {
    // Given
    String file1Id =
        seedFile("file1.txt", LOCAL_ROOT, "content".getBytes(StandardCharsets.UTF_8), OWNER_COOKIE);
  ...
  void givenAFlaggedFileWhenDeleteNodesThenItSucceeds() {
    ...
    String fileId =
        seedFile(
            "flagged-file.txt", LOCAL_ROOT, "content".getBytes(StandardCharsets.UTF_8), OWNER_COOKIE);
