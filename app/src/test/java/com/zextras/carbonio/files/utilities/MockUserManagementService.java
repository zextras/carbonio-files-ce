...
public class MockUserManagementService extends UserManagementServiceImplBase {
  ...
  /**
   * Reversible "user-management is unreachable" switch. The legacy {@code Simulator} physically
   * shut down the in-process gRPC server (one app per test class); under the shared {@code
   * @QuarkusTest} app the server is JVM-wide, so instead of killing it we flip this flag so every
   * RPC answers {@code UNAVAILABLE}. Cleared on {@code close()}/{@code reset()} so a later test
   * class is not poisoned. Backs {@code Mocks#userManagementDown()}.
   */
  ...
  public void getUserMyself(GetUserMyselfRequest request,
    ...
    if (down) {
      responseObserver.onError(
          Status.UNAVAILABLE.withDescription("user-management is down").asRuntimeException());
      return;
    }
